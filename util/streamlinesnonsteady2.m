...
function streamlinesnonsteady2(v, S, stepsize, maxit, cmap, lineWidth)
%STREAMLINES2 Plots integral curves in the plane.
%   
%   STREAMLINES2(v, S, stepsize, maxit, cmap, lineWidth) takes a vector
%   field v specified at P, and plots colored integral curves. The number
%   of steps is given by maxit. S are seed points of the integral curves
%   in R^2. cmap is used to interpolate colors. lineWidth is the line
%   width used for plotting.
%
...
for k=1:length(verts)
    ...
    for l=1:maxit
        ...
        F1 = griddedInterpolant(v(:, :, l, 1), 'linear', 'none');
        F2 = griddedInterpolant(v(:, :, l, 2), 'linear', 'none');
