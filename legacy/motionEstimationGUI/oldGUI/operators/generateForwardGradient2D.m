...
%
function [ Dx2D,Dy2D ] = generateForwardGradient2D( dims,stepsize )
    ex = ones(dims(1), 1);
    ey = ones(dims(2), 1);

    Dx = 1/stepsize(1) .* spdiags([-ex ex], 0:1, dims(1), dims(1));
    Dy = 1/stepsize(2) .* spdiags([-ey ey], 0:1, dims(2), dims(2)); 
